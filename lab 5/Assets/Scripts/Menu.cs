...
public class Menu : MonoBehaviour {
	...
	void OnGUI()
	{
		...
		if (! setting && !credit && !newGame) {
			...
			GUILayout.FlexibleSpace ();
			if (PlayerPrefs.GetInt ("Score") > 0) {
				if (GUILayout.Button ("Continua", GUILayout.Height (50))) {
			...
			if (GUILayout.Button ("About", GUILayout.Height (50))) {
				credit = true;
			}
		...
		if (credit) {
			GUILayout.FlexibleSpace();
			GUILayout.Label("[ BATALIA SPATIALA ]");
			GUILayout.FlexibleSpace();
			GUILayout.Label("Game Developer : Roman Chicu");

			GUILayout.Label("Music : Jay Hardway - Stardust");
			GUILayout.FlexibleSpace();
			if (GUILayout.Button ("< Back", GUILayout.Height (50))) {
				credit = false;
			}
